...
function getUrls(jsonData, nameList, source) {
	...
	Object.values(latestItems).forEach(item => {
		...
		const url = `${
			source === 'kkphim' ? 'https://phimapi.com/phim/' : 'https://ophim1.com/phim/'
		}${slug}|${id}|${modifiedTime}|${name}|${origin_name}|${year}`

		if (namesToCheck.includes(name.toLowerCase())) {
			matchingUrls.push({ name, url })
		} else {
			nonMatchingUrls.push({ name, url })
		}
	})
...
function updateCrawlSummary(jsonData) {
	const totalItems = jsonData.pagination.totalItems || 0
	const itemsPerPage = jsonData.pagination.totalItemsPerPage || 0
	const currentPage = jsonData.pagination.currentPage || 0
	const totalPages = jsonData.pagination.totalPages || 0
